/*****************************************************************************
 *
 * mng.h	Multiple Image Network Grapics - LC (low complexity)
 *
...
typedef enum {
	/** @brief MHDR tag is read: image buffer is allocated */
	MNG_INFO_MHDR,
	/** @brief TERM tag is read: terminate action, after, delay and maxiter are ready */
	MNG_INFO_TERM,
	/** @brief BACK tag is read: background color is ready */
	MNG_INFO_BACK,
	/** @brief pHYg tag is read: physical dimensions ready */
	MNG_INFO_PHYG,
	/** @brief tEXt tag is read: embedded text is ready (= parm) */
	MNG_INFO_TEXT,
	/** @brief PLTE tag is read: palette is decoded */
	MNG_INFO_PLTE,
	/** @brief tRNs tag is read: transparency is ready */
	MNG_INFO_TRNS,
	/** @brief tIME tag is read: time is set */
	MNG_INFO_TIME,
	/** @brief FRAM tag is read: frame buffer is modified; delay may be set */
	MNG_INFO_FRAM,
	/** @brief DEFI tag is read: xloc, yloc and the object clip rect are ready */
	MNG_INFO_DEFI,
	/** @brief IHDR tag is read: mng->img bitmap is updated with PNG */
	MNG_INFO_IHDR,
	/** @brief MEND tag is read: stream ends; exit if callback returns 1 */
	MNG_INFO_MEND
}	mng_info_t;
...
typedef	struct mng_s {
	 ...
	 * encoders should write 2^31-1, representing an infinite layer count.
	 */
	uint32_t lcount;
 ...
 * @param callback callback function to handle events while reading the stream
 * @result returns 0 on success, -1 on error
 */
extern int mng_read(const char *filename,
	void *cookie, int (*callback)(mng_t *mng, void *cookie, mng_info_t info, void *param));
 ...
 * @result returns 0 on success, -1 on error
 */
extern int mng_finish_png(mng_t *mng, int frame);
...
/**
 * @brief get the play time in seconds
 *
 * @param mng pointer to a mng_t context
 ...
 */
extern png_t *mng_append_png(mng_t *mng, int ifdelay, int x, int y, int w, int h, int color, int depth);
