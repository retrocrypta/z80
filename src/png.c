/*****************************************************************************
 *
 * png.c	Portable Network Graphics functions
 *
...
{
	xng_t *xng = (xng_t *)ptr;
	LOG((1,"XNG","xng_write_string(%p,'%s')\n",
		xng, src));
	if (NULL == xng || NULL == xng->output) {
		errno = EINVAL;
		return -1;
 ...
 * @param size maximum size of the string to read + 1
 * @result returns 0 on success, -1 on error
 */
int xng_read_string(void *ptr, char *dst, uint32_t size)
{
	...
	LOG((1,"XNG","xng_read_string(%p,%p,%#x)\n",
	...
	if (0 != (*xng->input)(xng->cookie, (uint8_t *)dst, size - 1))
		return -1;
	dst[size-1] = '\0';

	isocrc_bytes(&xng->crc, (uint8_t *)dst, size - 1);
...
int xng_write_bytes(void *ptr, uint8_t *buff, uint32_t size)
{
	...
	LOG((1,"XNG","xng_write_bytes(%p,%p,%d)\n",
		xng, buff, size));
...
int xng_read_bytes(void *ptr, uint8_t *buff, uint32_t size)
{
	...
	LOG((1,"XNG","xng_read_bytes(%p,%p,%#x)\n",
...
int xng_write_uint16(void *ptr, uint16_t val)
{
	...
	LOG((1,"XNG","xng_write_uint16(%p,%#x)\n",
		xng, (unsigned)val));
	if (NULL == xng || NULL == xng->output) {
		errno = EINVAL;
		return -1;
...
int xng_write_crc(void *ptr)
{
	...
	LOG((1,"XNG","xng_write_crc(%p) (crc:%#x)\n",
		xng, xng->crc));
...
int xng_read_crc(void *ptr)
{
	...
	LOG((1,"PNG","got CRC %08x, expect CRC %08x\n",
		crc, xng->crc));
	if (xng->crc != crc) {
		LOG((1,"PNG","CRC mismatch\n"));
		return -1;
	}
...
int png_read_IHDR(png_t *png, uint32_t size)
{
	uint32_t part;
	int rc;
	LOG((1,"PNG","png_read_IHDR(%p,%#x)\n",
		png, (unsigned)size));

	/* already got size and 'IHDR' tag */
	part = 13;
	if (size < part) {
		LOG((1,"PNG","size is less than %u\n", part));
		errno = EINVAL;
		return -1;
	}
	if (0 != (rc = xng_read_uint(png,&png->w))) {
		LOG((1,"PNG","failed to read width\n"));
		return rc;
	}
	if (0 != (rc = xng_read_uint(png,&png->h))) {
		LOG((1,"PNG","failed to read height\n"));
		return rc;
	}
	if (0 != (rc = xng_read_byte(png,&png->depth))) {
		LOG((1,"PNG","failed to read depth\n"));
		return rc;
	}
	if (0 != (rc = xng_read_byte(png,&png->color))) {
		LOG((1,"PNG","failed to read color mode\n"));
		return rc;
	}
	if (0 != (rc = xng_read_byte(png,&png->compression))) {
		LOG((1,"PNG","failed to read compression mode\n"));
		return rc;
	}
	if (0 != (rc = xng_read_byte(png,&png->filter))) {
		LOG((1,"PNG","failed to read filter mode\n"));
		return rc;
	}
	if (0 != (rc = xng_read_byte(png,&png->interlace))) {
		LOG((1,"PNG","failed to read interlace mode\n"));
		return rc;
	}
	if (size > part) {
		uint32_t i;
		LOG((5,"PNG","skipping %u bytes\n", size - part));
		for (i = part; i < size; i++) {
			uint8_t ignore;
...
int png_write_tEXt(png_t *png, const char *keyword, const char *text)
{
	uint32_t size = 0;
	int rc;
	LOG((1,"PNG","png_write_tEXt(%p,'%s','%s')\n",
		png, keyword, text));
...
int png_write_PLTE(png_t *png)
{
	...
	int rc;
	LOG((1,"PNG","png_write_PLTE(%p) (%u entries)\n",
		png, entries));
...
int png_read_PLTE(png_t *png, uint32_t size)
{
	...
	uint32_t entries = 1 << png->bpp;
	int rc;
	LOG((1,"PNG","png_read_PLTE(%pd) (max %u entries)\n",
		png, entries));

	if (entries > size / 3) {
		LOG((1,"PNG","read only %u of %u palette entries\n",
			size / 3, entries));
		entries = size / 3;
	}
	if (3 * entries < part) {
...
int png_write_tRNS(png_t *png)
{
	...
	LOG((1,"PNG","png_write_tRNS(%p) (%u bytes)\n",
		png, size));
	if (0 != (rc = xng_write_size(png, size)))	
		return rc;
...
int png_read_tRNS(png_t *png, uint32_t size)
{
	...
	LOG((1,"PNG","png_read_tRNS(%p) -> %u bytes\n",
		png, part));
	if (part > size) {
		part = size;
	}
...
int png_write_IDAT(png_t *png, uint8_t *idat, uint32_t size)
{
	...
	LOG((1,"PNG","png_write_IDAT(%p,%p,%#x)\n",
...
int png_read_IDAT(png_t *png, uint32_t size)
{
	...
	LOG((1,"PNG","png_read_IDAT(%p,%p,%#x) %#x/%#x\n",
...
int png_finish(png_t *png)
{
	...
	if (NULL == png->idat) {
		LOG((1,"PNG","malloc(%d) call failed (%s)\n",
	...
	if (Z_OK != (rc = compress2(png->idat, &gzsize, png->img, png->size, 9))) {
		LOG((1,"PNG","compress2(%p,%#x,%#p,%#x,%d) call failed (%d)\n",
			png->idat, gzsize,
...
/**
 * @brief read a PNG in an MNG stream
 *
 * @param fp file stream (MNG) to read from
 * @result returns pointer new png_t context on success, NULL on error
 */
png_t *png_read_stream(void *cookie, int (*input)(void *, uint8_t *,int))
{
	png_t *png = NULL;
	char tag[4+1];
	uint32_t size;
	...
	LOG((1,"PNG","png_read(%p,%p)\n", cookie, input));

	png = (png_t *)calloc(1, sizeof(png_t));
	if (NULL == png) {
		LOG((1,"PNG","calloc(%d,%d) call failed (%s)\n",
	...
	while (0 == iend) {
		if (0 != (rc = xng_read_size(png, &size))) {
			LOG((1,"PNG","read size failed (%s)\n",
				strerror(errno)));
			goto bailout;
		}
		if (0 != (rc = xng_read_string(png, tag, sizeof(tag)))) {
			LOG((1,"PNG","read tag failed (%s)\n",
				strerror(errno)));
			goto bailout;
		}
		LOG((5,"PNG","'%s' size %#x\n", tag, size));
		if (0 == strcmp(tag, "IHDR")) {
			...
			case COLOR_GRAYSCALE:
				...
				case 1:
					...
					png->bpp = 1;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_GRAYSCALE 1bpp: %#x\n",
						png->stride));
					break;
				case 2:
					...
					png->bpp = 2;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_GRAYSCALE 2bpp: %#x\n",
						png->stride));
					break;
				case 4:
					...
					png->bpp = 4;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_GRAYSCALE 4bpp: %#x\n",
						png->stride));
					break;
				case 8:
					png->stride = 1 + png->w;
					png->bpp = 8;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_GRAYSCALE 8bpp: %#x\n",
						png->stride));
					break;
				case 16:
					...
					png->bpp = 16;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_GRAYSCALE 16bpp: %#x\n",
						png->stride));
					break;
				default:
					LOG((1,"PNG","unsupported COLOR_GRAYSCALE depth %d\n",
						png->depth));
					rc = -1;
					goto bailout;
				}
			...
			case COLOR_RGBTRIPLE:
				...
				case 8:
					...
					png->bpp = 3 * 8;
					png->trns_size = 6;
					LOG((5,"PNG","COLOR_RGBTRIPLE 8bpp: %#x\n",
						png->stride));
					break;
				case 16:
					...
					png->bpp = 3 * 16;
					png->trns_size = 6;
					LOG((5,"PNG","COLOR_RGBTRIPLE 16bpp: %#x\n",
						png->stride));
					break;
				default:
					LOG((1,"PNG","unsupported COLOR_RGBTRIPLE depth %d\n",
						png->depth));
					rc = -1;
					goto bailout;
				}
			...
			case COLOR_PALETTE:
				...
				case 1:
					...
					png->bpp = 1;
					png->trns_size = 2;
					LOG((5,"PNG","COLOR_PALETTE 1bpp: %#x\n",
						png->stride));
					break;
				case 2:
					png->stride = 1 + (png->w + 3) / 4;
					png->bpp = 2;
					png->trns_size = 4;
					LOG((5,"PNG","COLOR_PALETTE 2bpp: %#x\n",
						png->stride));
					break;
				case 4:
					png->stride = 1 + (png->w + 1) / 2;
					png->bpp = 4;
					png->trns_size = 16;
					LOG((5,"PNG","COLOR_PALETTE 4bpp: %#x\n",
						png->stride));
					break;
				case 8:
					png->stride = 1 + png->w;
					png->bpp = 8;
					png->trns_size = 256;
					LOG((5,"PNG","COLOR_PALETTE 8bpp: %#x\n",
						png->stride));
					break;
				default:
					LOG((1,"PNG","unsupported COLOR_PALETTE depth %d\n",
						png->depth));
					rc = -1;
					goto bailout;
				}
			...
			case COLOR_GRAYALPHA:
				...
				case 8:
					...
					png->bpp = 2 * 8;
					png->trns_size = 0;
					LOG((5,"PNG","COLOR_GRAYALPHA 8bpp: %#x\n",
						png->stride));
					break;
				case 16:
					...
					png->bpp = 2 * 16;
					png->trns_size = 0;
					LOG((5,"PNG","COLOR_GRAYALPHA 16bpp: %#x\n",
						png->stride));
					break;
				default:
					LOG((1,"PNG","unsupported COLOR_GRAYALPHA depth %d\n",
						png->depth));
					rc = -1;
					goto bailout;
				}
			...
			case COLOR_RGBALPHA:
				...
				case 8:
					...
					png->bpp = 4 * 8;
					png->trns_size = 0;
					LOG((5,"PNG","COLOR_RGBALPHA 8bpp: %#x\n",
						png->stride));
					break;
				case 16:
					...
					png->bpp = 4 * 16;
					png->trns_size = 0;
					LOG((5,"PNG","COLOR_RGBALPHA 16bpp: %#x\n",
						png->stride));
					break;
				default:
					LOG((1,"PNG","unsupported COLOR_RGBALPHA depth %d\n",
						png->depth));
					rc = -1;
					goto bailout;
				}
				break;
			default:
				LOG((1,"PNG","unsupported color model %d, depth %d\n",
					png->color, png->depth));
				rc = -1;
				goto bailout;
			}
			png->size = png->h * png->stride;
			LOG((5,"PNG","image size is %#x, %ux%ux%u, %u bytes/row\n",
			...
			if (NULL == png->idat) {
				LOG((1,"PNG","malloc/realloc(%d) call failed (%s)\n",
					png->isize, strerror(errno)));
				rc = -1;
				goto bailout;
			}
			if (0 != (rc = png_read_IDAT(png,size))) {
				LOG((1,"PNG","read 'IDAT' failed (%s)\n",
					strerror(errno)));
				free(png->idat);
				goto bailout;
			}
			...
			if (png->isize > 0) {
				...
				if (Z_OK != rc) {
					LOG((1,"PNG","uncompress(%p,%#x,%p,%#x) failed (%d)\n",
						png->img, (unsigned)gzsize,
						png->idat, (unsigned)png->isize, rc));
					goto bailout;
				}
				LOG((3,"PNG","uncompress(%p,%#x,%p,%#x) success (%d)\n",
					png->img, (unsigned)gzsize,
					png->idat, (unsigned)png->isize, rc));
				png->offs += gzsize;
			}
		} else {
			LOG((1,"PNG","ignore tag '%s'\n", tag));
			for (i = 0; i < size; i++) {
				uint8_t ignore;
				xng_read_byte(png, &ignore);
			}
			if (0 != (rc = xng_read_crc(png))) {
				LOG((1,"PNG","CRC of unknown tag '%s' failed\n",
					tag));
				goto bailout;
			}
		}
	...
	for (y = 0; y < (uint32_t)png->h; y++) {
		...
		case 4:
			...
			break;
		default:
			LOG((1,"PNG","unknown filter %#x in row %d\n",
				row[0], y));
		}
	}
	...
	void *cookie, int (*output)(void *cookie, uint8_t *data, int size))
{
	...
	LOG((1,"PNG","png_read('%s')\n", filename));

	fp = fopen(filename, "rb");
	if (NULL == fp) {
		LOG((1,"PNG","fopen('%s','rb') call failed (%s)\n",
	...
	if (0 != (*png->x.input)(png->x.cookie, magic, sizeof(magic))) {
		LOG((1,"PNG","read magic failed\n"));
		free(png);
		fclose(fp);
		return NULL;
	}
	if (memcmp(magic, pngmagic, sizeof(magic))) {
		LOG((1,"PNG","magic mismatch\n"));
		free(png);
		fclose(fp);
		return NULL;
	}
	...
	png = png_read_stream(fp, fp_read_bytes);
	if (NULL == fp) {
		LOG((1,"PNG","read_stream(%p,%p) failed\n", fp, fp_read_bytes));
		rc = -1;
		fclose(fp);
		goto bailout;
	}
	...
	void *cookie, int (*output)(void *cookie, uint8_t *data, int size))
{
	...
	LOG((1,"PNG","png_create(%d,%d,%d,%d,%p,%p)\n",
	...
	case COLOR_GRAYSCALE:
		...
		LOG((1,"PNG","Grayscale bpp:%d stride:%d\n",
			png->bpp, png->stride));
		break;
	case COLOR_RGBTRIPLE:
		switch (depth) {
		...
		LOG((1,"PNG","RGB triple bpp:%d stride:%d\n",
			png->bpp, png->stride));
		break;
	case COLOR_PALETTE:
		switch (depth) {
		...
		LOG((1,"PNG","Palette bpp:%d stride:%d\n",
			png->bpp, png->stride));
		break;
	case COLOR_GRAYALPHA:
		switch (depth) {
		...
		LOG((1,"PNG","Gray + Alpha bpp:%d stride:%d\n",
			png->bpp, png->stride));
		break;
	case COLOR_RGBALPHA:
		switch (depth) {
		...
		LOG((1,"PNG","RGB + Alpha bpp:%d stride:%d\n",
			png->bpp, png->stride));
		break;
	default:
		free(png);
	...
	LOG((1,"PNG","image is %d bytes (width:%d height:%d stride:%d)\n",
...
int png_put_pixel(png_t *png, int x, int y, int color, int alpha)
{
	...
	if (x < 0 || (uint32_t)x >= png->w || y < 0 || (uint32_t)y >= png->h) {
		LOG((5,"PNG","png_put_pixel(%p,%d,%d,%d,%d) out of bounds\n",
			png, x, y, color, alpha));
		return 0;
	}
...
int png_get_pixel(png_t *png, int x, int y, int *color, int *alpha)
{
	...
	*alpha = 0;
	...
	if (x < 0 || (uint32_t)x >= png->w || y < 0 || (uint32_t)y >= png->h) {
		LOG((5,"PNG","png_get_pixel(%p,%d,%d,%d,%d) out of bounds\n",
			png, x, y, color, alpha));
		return 0;
	}
...
/**
 * @brief find a matching or the best matching palette entry for a color
 *
 * @param cookie pointer to a png_t context
 * @param ncolors number of colors in the png->pal
 * @param color RGB color in PNG format (rrrrrrrrggggggggbbbbbbbb)
 * @result returns the (best) matching palette index
 */
static uint32_t find_palette(void *cookie, uint32_t ncolors, uint32_t color)
{
	static int hit;
	png_t *png = (png_t *)cookie;
	int diff, best_diff;
	uint32_t n, best;
	...
	for (n = hit; n < ncolors; n++) {
		uint8_t *p = &png->pal[3*n];
	...
	for (n = 0, best = 0, best_diff = 99999999; n < ncolors; n++) {
		uint8_t *p = &png->pal[3*n];
		diff =	(r - p[0]) * (r - p[0]) +
			(g - p[1]) * (g - p[1]) +
			(b - p[2]) * (b - p[2]);
...
/**
 * @brief bit block transfer from XXX source
 *
 * @param png pointer to a png_t context
...
#define	PNG_BLIT_FROM(SRC) \
int png_blit_from_##SRC(png_t *png, int dx, int dy, int sx, int sy, \
	int w, int h, uint8_t *src, int stride, \
	uint32_t *colors, int alpha) \
{ \
	switch (png->color) { \
	case COLOR_GRAYSCALE: \
		switch (png->depth) { \
	...
	case COLOR_GRAYALPHA: \
		switch (png->depth) { \
		case 8: \
			blit_##SRC##_to_graya8( \
				png->img + 1, dx, dy, png->stride, \
				src, sx, sy, stride, \
				w, h, colors, alpha); \
			break; \
		case 16: \
			blit_##SRC##_to_graya16( \
				png->img + 1, dx, dy, png->stride, \
				src, sx, sy, stride, \
				w, h, colors, alpha); \
			break; \
	...
	case COLOR_PALETTE: \
		...
		case 1: \
			blit_##SRC##_to_pal1( \
				png->img + 1, dx, dy, png->stride, \
				src, sx, sy, stride, \
				w, h, colors, alpha, png, find_palette); \
			break; \
		case 2: \
			blit_##SRC##_to_pal2( \
				png->img + 1, dx, dy, png->stride, \
...
PNG_BLIT_FROM(gray4)
PNG_BLIT_FROM(gray8)
PNG_BLIT_FROM(gray16)

PNG_BLIT_FROM_PAL(pal1)
PNG_BLIT_FROM_PAL(pal2)
PNG_BLIT_FROM_PAL(pal4)
PNG_BLIT_FROM_PAL(pal8)

PNG_BLIT_FROM(graya8)
PNG_BLIT_FROM(graya16)

PNG_BLIT_FROM_PAL(rgb8)
PNG_BLIT_FROM_PAL(rgb16)

PNG_BLIT_FROM_PAL(rgba8)
PNG_BLIT_FROM_PAL(rgba16)
