...
/***************************************************************************************
 *
 * z80dasm.c	Z80 CPU disassembler
 *
...
static const z80dasm_t mnemonic_xx_cb[256] =
{
...
static const z80dasm_t mnemonic_cb[256] =
{
...
static const z80dasm_t mnemonic_ed[256] =
{
	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},
	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},
	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},
...
static const z80dasm_t mnemonic_xx[256] =
{
	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},
	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},	{zDB,"?"},
...
static const z80dasm_t mnemonic_main[256] =
{
	{zNOP,NULL},	{zLD,"bc,N"},   {zLD,"(bc),a"}, {zINC,"bc"},
	{zINC,"b"},     {zDEC,"b"},     {zLD,"b,B"},    {zRLCA,NULL},
	{zEX,"af,af'"}, {zADD,"hl,bc"}, {zLD,"a,(bc)"}, {zDEC,"bc"},
	{zINC,"c"},     {zDEC,"c"},     {zLD,"c,B"},    {zRRCA,NULL},
	{zDJNZ,"O"},    {zLD,"de,N"},   {zLD,"(de),a"}, {zINC,"de"},
	{zINC,"d"},     {zDEC,"d"},     {zLD,"d,B"},    {zRLA,NULL},
	{zJR,"O"},	{zADD,"hl,de"}, {zLD,"a,(de)"}, {zDEC,"de"},
	{zINC,"e"},     {zDEC,"e"},     {zLD,"e,B"},    {zRRA,NULL},
	{zJR,"nz,O"},   {zLD,"hl,N"},   {zLD,"(W),hl"}, {zINC,"hl"},
	{zINC,"h"},     {zDEC,"h"},     {zLD,"h,B"},    {zDAA,NULL},
	{zJR,"z,O"},    {zADD,"hl,hl"}, {zLD,"hl,(W)"}, {zDEC,"hl"},
	{zINC,"l"},     {zDEC,"l"},     {zLD,"l,B"},    {zCPL,NULL},
	{zJR,"nc,O"},   {zLD,"sp,N"},   {zLD,"(W),a"},  {zINC,"sp"},
	{zINC,"(hl)"},  {zDEC,"(hl)"},  {zLD,"(hl),B"}, {zSCF,NULL},
	{zJR,"c,O"},    {zADD,"hl,sp"}, {zLD,"a,(W)"},  {zDEC,"sp"},
	{zINC,"a"},     {zDEC,"a"},     {zLD,"a,B"},    {zCCF,NULL},
	...
	{zLD,"(hl),h"}, {zLD,"(hl),l"}, {zHLT,NULL},	{zLD,"(hl),a"},
	...
	{zRET,"p"},     {zPOP,"af"},    {zJP,"p,A"},    {zDI,NULL},
	{zCALL,"p,A"},  {zPUSH,"af"},   {zOR,"B"},	{zRST,"V"},
	{zRET,"m"},     {zLD,"sp,hl"},  {zJP,"m,A"},    {zEI,NULL},
...
static char *hexb(uint32_t val)
{
	static char buff[4][8];
	static int which;
	char *dst;
	...
	dst = buff[which];
	if (val >= 0xa0) {
		snprintf(dst, 8, "%03XH", val);
		return dst;
	}
	if (val >= 0x0a) {
		snprintf(dst, 8, "%02XH", val);
		return dst;
	}
	snprintf(dst, 8, "%u", val);
	return dst;
}

static char *hexw(uint32_t val)
{
	static char buff[4][8];
	static int which;
	char *dst;
	...
	dst = buff[which];
	if (val >= 0xa000) {
		snprintf(dst, 8, "%05XH", val);
		return dst;
	}
	if (val >= 0x0a) {
		snprintf(dst, 8, "%04XH", val);
		return dst;
	}
	snprintf(dst, 8, "%u", val);
	return dst;
 ...
 */
uint32_t z80_dasm(char *buffer, uint32_t pc, const uint8_t *oprom, const uint8_t *opram)
{
	const z80dasm_t *d;
	const char *src;
	const char *ixy;
	...
	if (d->parameters) {
		dst += sprintf(dst, "%-7s ", s_mnemonic[d->mnemonic]);
		src = d->parameters;
		while (*src) {
			switch (*src) {
			case '?':   /* illegal opcode */
				dst += sprintf( dst, "%s,%s", hexb(op), hexb(op1));
				break;
			case 'A':
				ea = opram[pos+0] + 256 * opram[pos+1];
				pos += 2;
				dst += sprintf(dst, "%s", hexw(ea));
				break;
			case 'B':   /* Byte op arg */
				ea = opram[pos++];
				dst += sprintf(dst, "%s", hexb(ea));
				break;
			...
			case 'X':
				...
				/* fall through */
			case 'Y':
				dst += sprintf( dst,"(%s%c%s)", ixy,
					sign(offset), hexb(offs(offset)));
				break;
			case 'I':
	...
	/* convert the whole shebang upper case */
	dst = buffer;
	while (*dst) {
		*dst = toupper((uint8_t)*dst);
		dst++;
	}
